...
matrix matrix::operator* (const int v) {
	...
	if (!a) return *out;
	out->make (this->n, this->m);
	for (int i=0; i<n; i++)
		for (int j=0; j<m; j++)
			out->a[i][j]=this->a[i][j]*v;
	return *out;
}

matrix& matrix::operator= (const matrix& sourse) {
	if (&sourse==this) return *this;
	make (sourse.n, sourse.m);
	for (int i=0; i<n; i++)
		for (int j=0; j<m; j++)
			this->a[i][j]=sourse.a[i][j];
	return *this;
}

//ее надо дружественной
